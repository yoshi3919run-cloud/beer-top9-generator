        ...
        with st.spinner('プレイボール！...'):
            ...
            for order in range(1, 10):
                ...
                if order in images:
                    ...
                    # ブルワリーとビール名を繋げて、2行目以降もスムーズに流れるようにする
                    clean_label = labels[order].replace(" / ", " ").replace("/", " ")
                    display_text = f"{order}. {prefix} {clean_label}"
            ...
            st.download_button(label="📥 スコアボードを保存", data=buf.getvalue(), file_name="beer_scoreboard.png", mime="image/png")
